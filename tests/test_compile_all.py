...
def test_example_compiles(yaml_path):
    ...
    # docs/design_decisions.md, point 22. Plus AUCUN front n'est
    # généré depuis le pivot (point 41) — voir le dernier test du fichier.
...
def test_no_example_ever_produces_frontend_html():
    ...
    for yaml_path in EXAMPLE_FILES:
        ...
        MonlSecureGenerator(normalized_ast).generate_all()
...
def test_no_example_ever_produces_any_generated_frontend():
    ...
    for yaml_path in EXAMPLE_FILES:
        ...
        normalized_ast = MonlAST(raw_json).validate_and_audit()
        MonlSecureGenerator(normalized_ast).generate_all()
        for ghost in ("landing.html", "dashboard.html", "frontend.html"):
            assert not os.path.exists(os.path.join(base_dir, ghost)), (
